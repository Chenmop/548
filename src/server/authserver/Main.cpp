...
{
	SetConsoleTitle("CaffCore Authentication Server");
	...
	sLog->outInfo(LOG_FILTER_AUTHSERVER, "CaffCore Authentication Server");
	sLog->outInfo(LOG_FILTER_AUTHSERVER, "Supported World Of Warcraft client build is 5.4.8 18414\n");
...
#ifdef _WIN32 // Windows
    {
        ...
        {			
			if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
			{
				...
				sLog->outInfo(LOG_FILTER_AUTHSERVER, "CaffCore Auth Server set to HIGH");
			}
			else
			{
    ...
    while (!stopEvent)
    {
        ...
        {
            loopCounter = 0;
            sLog->outInfo(LOG_FILTER_AUTHSERVER, "Pinging MySQL to keep connection alive");
